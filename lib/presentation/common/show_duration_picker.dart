...
class _DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(centerPoint, radius * 0.88, new Paint()..color = Colors.white);
    ...
    TextPainter textDurationValuePainter = new TextPainter(
        ...
        text: new TextSpan(
            ...
            style: Theme.of(context).textTheme.display3.copyWith(fontSize: size.shortestSide * 0.15)),
        textDirection: TextDirection.ltr)
      ..layout();
    Offset middleForValueText = new Offset(
        centerPoint.dx - (textDurationValuePainter.width / 2), centerPoint.dy - textDurationValuePainter.height / 2);
    ...
    textMinPainter.paint(
        canvas,
        new Offset(centerPoint.dx - (textMinPainter.width / 2),
...
class _Dial extends StatefulWidget {
  const _Dial({@required this.duration, @required this.onChanged, this.snapToMins = 1.0}) : assert(duration != null);
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _theta = _thetaTween.animate(new CurvedAnimation(parent: _thetaController, curve: Curves.fastOutSlowIn))
  ...
  Duration _getTimeForTheta(double theta) {
    ...
    fractionalRotation = fractionalRotation < 0 ? 1 - fractionalRotation.abs() : fractionalRotation;
  ...
  void _updateRotations(double oldTheta, double newTheta) {
    ...
    if (oldTheta > _kCircleTop && newTheta <= _kCircleTop && oldTheta < _kCircleLeft) {
      setState(() => _hours = _hours + 1);
      // If the angle cross anti-clockwise through 12'o'clock
    } else if (oldTheta <= _kCircleTop && newTheta > _kCircleTop && newTheta < _kCircleBottom) {
      if (_hours > 0) {
        setState(() => _hours = _hours - 1);
      }
    }
...
Future<Duration> showDurationPicker(
    {@required BuildContext context, @required Duration initialTime, double snapToMins}) async {
  assert(context != null);
  assert(initialTime != null);
...
class DurationPicker extends StatelessWidget {
  ...
  DurationPicker(
      {this.duration = const Duration(minutes: 0), @required this.onChange, this.snapToMins, this.width, this.height});
